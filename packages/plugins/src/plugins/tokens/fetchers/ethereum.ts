...
const executor: FetcherExecutor = async (owner: string, cache: Cache) => {
  const client = getEvmClients().ethereum;

  const tokenList = await cache.getItem<TokenList>(NetworkId.ethereum, {
    prefix: 'tokenList',
  });
  ...
  const results = await runInBatch(
    tokensContracts.map(
      (contract) => () => cache.getTokenPrice(contract, NetworkId.ethereum)
    )
  );
...
const fetcher: Fetcher = {
  id: `${walletTokensPlatform.id}-ethereum`,
  networkId: NetworkId.ethereum,
  executor,
};
