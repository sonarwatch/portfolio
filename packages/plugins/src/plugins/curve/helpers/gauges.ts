...
export async function getOwnerGaugeRewards(
  ...
  const gaugeRewardsByTokenAddressRes = (await client.multicall({
    contracts: rewardTokenBalanceCalls,
  })) as { result: bigint }[];

  const gaugeRewardsByTokenAddress = gaugeRewardsByTokenAddressRes.map(
    (result, index) => ({
      address: rewardTokenBalanceCalls[index].args[1],
      balance: new BigNumber(result.result.toString()),
    })
  );

  return gaugeRewardsByTokenAddress;
}
