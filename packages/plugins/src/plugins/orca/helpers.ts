...
export function tickIndexToSqrtPricePositive(tick: number): BN {
  let ratio;
...
export function tickIndexToSqrtPriceNegative(tickIndex: number): BN {
...
export function orcaTickToPriceX64(tickIndex: number) {
  if (tickIndex > 0) {
    return new BigNumber(tickIndexToSqrtPricePositive(tickIndex).toString());
  }
  return new BigNumber(tickIndexToSqrtPriceNegative(tickIndex).toString());
}

const fromX64Decimal = (num: Decimal) => num.times(Decimal.pow(2, -64));
const toX64Decimal = (num: Decimal) => num.times(Decimal.pow(2, 64));

export function getOrcaTokenAmountsFromLiquidity(
  liquidity: number,
