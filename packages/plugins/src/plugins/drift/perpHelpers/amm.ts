...
import { clampBN, sigNum, squareRootBN } from './math';
import { assert } from './assert';
import { calculateLiveOracleStd, getNewOracleConfPct } from './oracle';
import {
...
export function calculateSpreadReserves(
  ...
  if (amm.curveUpdateIntensity > 100) {
    ...
    referencePriceOffset = calculateReferencePriceOffset(
      ...
      new BN(amm.historicalOracleData.lastOraclePriceTwap5Min.toString(10), 10),
      amm.lastMarkPriceTwap5Min,
      new BN(amm.historicalOracleData.lastOraclePriceTwap.toString(10), 10),
 ...
 */
export function calculateTerminalPrice(market: PerpMarket) {
...
export function calculateSpread(
  ...
  if (!Array.isArray(spreads)) return [0, 0];
...
function calculatePegFromTargetPrice(
  targetPrice: BN,
  baseAssetReserve: BN,
  quoteAssetReserve: BN
): BN {
